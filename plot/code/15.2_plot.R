...
p2 <- ggplot(data=pd2, aes(x = sc, y = bulk)) +
  geom_point(color = 'black', size = 0.2, alpha = 0.5) + 
  theme_classic() +
  xlab('scVI imputed single cells') + ylab('bulk (FPKM)') +
  theme(axis.title.x=element_text(size=12),axis.title.y=element_text(size=12))+
...
pdf('./plot/plot/imp_diff_eval_4subfig.pdf',width=9,height=6)
grid.arrange(p1,p2,p3,p4,layout_matrix=matrix(c(1,3,3,3,2,3,3,3,rep(4,12)),4))
dev.off()
