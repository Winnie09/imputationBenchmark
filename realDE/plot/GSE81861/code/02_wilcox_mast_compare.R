...
library(ggplot2)
library(ggrepel)
ove = readRDS('./realDE/result/GSE81861/overlap/mast/bulk_sc_diffgene_overlaps.rds')
ove = ove[rowMeans(is.na(ove))<1, ]
o1 <- rowMeans(ove)
ove = readRDS('./realDE/result/GSE81861/overlap/wilcox/bulk_sc_diffgene_overlaps.rds')
ove = ove[rowMeans(is.na(ove))<1, ]
o2 <- rowMeans(ove)
...
pdf('./realDE/plot/GSE81861/plot/wilcox_mast_compare.pdf',width=4,height=4)
