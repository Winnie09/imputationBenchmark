for (diffmtd in c('wilcox','mast')){
  ...
  ove_high =  readRDS(paste0(rdir,'bulk_sc_diffgene_overlaps_highExprGene.rds'))
  ove_low = readRDS(paste0(rdir,'bulk_sc_diffgene_overlaps_lowExprGene.rds'))
  ...
  p1 <- ggplot(data=pd_high,aes(x=data,y=method,fill=overlap)) + geom_tile()+ 
    ...
    theme(axis.text.x = element_text(angle=90)) + labs(fill='overlap') + ggtitle(paste0(diffmtd,',highExprGene')) +
  ...
  p2 <- ggplot(data=pd_low,aes(x=data,y=method,fill=overlap)) + geom_tile()+ 
    scale_fill_gradient(high = "#132B43", low = "#56B1F7",limits=c(0,1)) +
    theme_minimal() + xlab('') + ylab('')+
    theme(axis.text.x = element_text(angle=90)) + labs(fill='overlap') + ggtitle(paste0(diffmtd,',lowExprGene')) +
    theme(axis.text.y = element_text(color=ifelse(levels(pd_low$method)=='raw','red','black')))
  
  pdf(paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/rna_imputation/realDE/plot/GSE81861/plot/',diffmtd,'/overlap_hm_highLowExprGene.pdf'),height=6,width=10)
