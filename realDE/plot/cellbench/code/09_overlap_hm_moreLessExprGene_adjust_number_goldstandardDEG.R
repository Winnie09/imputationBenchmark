...
num.gs <- readRDS('/home-4/whou10@jhu.edu/scratch/Wenpin/rna_imputation/realDE/result/cellbench/bulkdiff/number_of_gold_standard_DEG.rds')
for (diffmtd in c('wilcox','mast')){
  ...
  ## adjust by number of gold standard DEG
  ove_high <- ove_high/(num.gs[colnames(ove_high)]/1e4)
  ove_low <- ove_low/(num.gs[colnames(ove_low)]/1e4)
  ...
  pdf(paste0('./realDE/plot/cellbench/plot/',diffmtd,'/overlap_hm_moreLessExprGene_adjust_number_goldstandardDEG.pdf'),height=4.8,width=15)
