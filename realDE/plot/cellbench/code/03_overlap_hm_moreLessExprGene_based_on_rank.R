...
for (diffmtd in c('wilcox','mast')){
  ...
  ####### use rank instead of values ######
  tmp <- apply(ove_high, 2, rank)
  rownames(tmp) <- rownames(ove_high)
  ove_high <- tmp
  tmp <- apply(ove_low, 2, rank)
  rownames(tmp) <- rownames(ove_low)
  ove_low <- tmp
  #########
  mtdorder1 = names(sort(rowMeans(ove_high,na.rm=T)))
  mtdorder2 = names(sort(rowMeans(ove_low,na.rm=T)))
  ...
  pdf(paste0('./realDE/plot/cellbench/plot/',diffmtd,'/overlap_hm_moreLessExprGene_based_on_rank.pdf'),height=4.8,width=15)
