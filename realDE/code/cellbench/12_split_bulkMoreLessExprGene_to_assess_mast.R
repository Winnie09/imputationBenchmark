allmtd = list.files('/home-4/whou10@jhu.edu/scratch/Wenpin/rna_imputation/realDE/result/cellbench/diff/mast/')
mtd = allmtd[1]
allf = list.files(paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/rna_imputation/realDE/result/cellbench/diff/mast/',mtd,'/res/'))
...
ove <- sapply(allmtd, function(mtd){
  ...
  t(sapply(allf,function(f) {
    print(f)
    if (file.exists(paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/rna_imputation/realDE/result/cellbench/diff/mast/',mtd,'/res/',f))){
      ...
      fc <- abs(fc)
      highid = names(which(fc>quantile(fc,0.9))) 
      lowid = names(which(fc<quantile(fc,0.1))) 
...
rdir = '/home-4/whou10@jhu.edu/scratch/Wenpin/rna_imputation/realDE/result/cellbench/overlap/mast/'
