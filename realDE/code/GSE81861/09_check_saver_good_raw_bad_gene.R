...
ranklist <- lapply(allf,function(f){
  ...
  res = readRDS(paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/rna_imputation/realDE/result/GSE81861/diff/wilcox/raw/res/',f))
  res = res[order(res$fdr),]
  rawrank <- 1:nrow(res)
  names(rawrank) <- res[,"Gene"]
  rawrank <- rawrank[intersect(names(rawrank),gs)]
  
  res = readRDS(paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/rna_imputation/realDE/result/GSE81861/diff/wilcox/saver/res/',f))
  res = res[order(res[,'fdr']),]
  srank <- 1:nrow(res)
...
res <- sapply(allf,function(f){
  print(f)  
  imp = readRDS(paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/rna_imputation/result/procimpute/GSE81861/raw/GSE81861_Cell_Line_COUNT.rds'))
  i = ranklist[[f]]
