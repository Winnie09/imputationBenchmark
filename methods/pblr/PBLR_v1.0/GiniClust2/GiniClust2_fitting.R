...
# .1 load data
load(paste("GiniClust2/results/", exprimentID, "_ExprM.filter.RData", sep=""))
...
if(gini.bi==0){
  ...
  #dim(GiniIndex) 
  write.table(GiniIndex, file=paste("GiniClust2/results/", exprimentID,"_bi-directional.GiniIndexTable.csv", sep=""), sep=",", row.names = TRUE,  col.names = TRUE,  quote = FALSE) 
}
...
pdf(paste("GiniClust2/figures/", exprimentID, "_histogram of Normalized.Gini.Score.pdf", sep=""), width=6, height=6)
...
# .6 save results
save(Genelist.top_pvalue, Genelist.HighNormGini, ExprM.Stat2, file=paste("GiniClust2/results/", exprimentID,"_StatisticsTable_afterLOESS.RData",sep=""))
...
if(dim(GiniIndex)[2] > 2){
  ...
  write.csv(Gini_related_table_qPCR, file="GiniClust2/results/Gini_related_table_qPCR.csv")
  
  pdf(paste("GiniClust2/figures/", exprimentID, "_smoothScatter_normGini_genes.pdf", sep=""), width=6, height=6, useDingbats = FALSE)
  ...
  write.csv(Gini_related_table_RNAseq, file="GiniClust2/results/Gini_related_table_RNA-seq.csv")
  
  pdf(paste("GiniClust2/figures/", exprimentID, "_smoothScatter_pvalue_gene.pdf", sep=""), width=6, height=6, useDingbats = FALSE)
